tepBADA.inference.battery <- function(DATA, scaleFlag = TRUE, centerFlag = TRUE, DESIGN = NULL, make_design_nominal = TRUE, group.masses = NULL, ind.masses = NULL, weights = NULL, graphs = TRUE, k = 0, test.iters = 100, critical.value = 2){
	
	res <- tepBADA(DATA = DATA, scaleFlag = scaleFlag, centerFlag = centerFlag, DESIGN = DESIGN, make_design_nominal = make_design_nominal, group.masses = group.masses, ind.masses = ind.masses, weights = weights, graphs = graphs, k = k)
	...
	for(i in 1:nrow(DATA)){
		loo.test.res <- loo.test(DATA,DESIGN,scaleFlag,centerFlag,i)
	...
	for(i in 1:test.iters){
		boot.res <- boot.compute.cubes(DATA,DESIGN,res$TExPosition.Data,scaleVals=res$TExPosition.Data$scale,centerVals=res$TExPosition.Data$center)
		#}

		FBX[,,i] <- boot.res$FBX
		FBY[,,i] <- boot.res$FBY
		permute.res <- permute.tests(DATA = DATA, scaleFlag = scaleFlag, centerFlag = centerFlag, DESIGN = DESIGN, make_design_nominal = make_design_nominal, group.masses = group.masses, ind.masses = ind.masses, weights = weights, k = k)
...
loo.test <- function(DATA,DESIGN,scaleFlag=TRUE,centerFlag=TRUE,i){
	Xminus1 <- DATA[-i,]
	Yminus1 <- DESIGN[-i,]
	massedY <- t(t(Yminus1) * (1/(colSums(Yminus1))))
	Rminus1 <- t(massedY) %*% Xminus1
	Rminus1 <- scale(Rminus1, scale = scaleFlag, center = centerFlag)
	BADAminus1 <- tepBADA(DATA=Xminus1,DESIGN=Yminus1, make_design_nominal=FALSE,centerFlag=centerFlag, scaleFlag=scaleFlag,graphs=FALSE)
	
	supX <- supplementaryRows(SUP.DATA=t(DATA[i,]), res=BADAminus1, center=BADAminus1$TExPosition.Data$center, scale=BADAminus1$TExPosition.Data$scale)
	assignSup <- fii2fi(DESIGN=t(DESIGN[i,]), fii=supX$fii, fi=BADAminus1$fi)
	return(list(assignSup=assignSup,supX=supX))
}

permute.tests <- function(DATA, scaleFlag = TRUE, centerFlag = TRUE, DESIGN = NULL, make_design_nominal = TRUE, group.masses = NULL, ind.masses = NULL, weights = NULL, k = 0){
	
	PermDATA <- DATA[sample(nrow(DATA),nrow(DATA),FALSE),]
	perm.res <- tepBADA(DATA=PermDATA, scaleFlag, centerFlag, DESIGN, make_design_nominal, group.masses, ind.masses, weights, graphs = FALSE, k)
	perm.eigs <- perm.res$eigs
	perm.r2 <- perm.res$assign$r2
