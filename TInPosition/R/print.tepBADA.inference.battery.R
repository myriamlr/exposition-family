...
#list(fi=fi,di=di,ci=ci,ri=ri,fj=fj,cj=cj,rj=rj,dj=dj,t=taus,M=M,W=W,pdq=gpdq_results)
  ...
  res[15,] <- c("$fii","Factor scores of the individuals")
  res[16,] <- c("$dii","Squared distances of the individuals")
  res[17,] <- c("$rii", "Cosines of the individuals")
  res[18,] <- c("$assign","Information for assignment of individuals to groups")  
