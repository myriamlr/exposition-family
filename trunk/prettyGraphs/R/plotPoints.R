...
function(data_matrix,col,x_axis=1,y_axis=2,cex=1,pch=21,contributionCircles=FALSE,contributions=NULL,flip=FALSE){
	if(contributionCircles){
		if(is.null(contributions)){
			...
			if(flip){
				points(data_matrix[,x_axis],data_matrix[,y_axis],col=col,pch=pch,bg="black",cex=cex)			
			}else{
				points(data_matrix[,x_axis],data_matrix[,y_axis],col="black",pch=pch,bg=col,cex=cex)
			}
		}else{
			...
			thesecontributions[which(thesecontributions > 3)] <- 3
				#why does the above happen? It shouldn't.
			#if(is.null(pch)){
			#	if(flip){
