...
#		}
		if( !is.null(contributions) && !(nrow(as.matrix(contributions))!=nrow(data_matrix)) ){
			...
			thesecontributions[which(thesecontributions > 1.5)] <- 1.5
			text.cex <- as.matrix(text.cex * thesecontributions)
		}
	}
	if(pos < 1 || pos > 4){
		text(data_matrix[,x_axis],data_matrix[,y_axis],col=col,labels=rownames(data_matrix),cex=text.cex,pos=3)
	}else{
		text(data_matrix[,x_axis],data_matrix[,y_axis],col=col,labels=rownames(data_matrix),cex=text.cex,pos=pos)
	}
}
