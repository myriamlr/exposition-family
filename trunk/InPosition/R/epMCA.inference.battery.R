...
epMCA.inference.battery <- function(DATA, make_data_nominal = TRUE, DESIGN = NULL, make_design_nominal = TRUE, masses = NULL, weights = NULL, hellinger = FALSE, symmetric = TRUE, correction = c("b"), graphs = TRUE, k = 0, test.iters=100, constrained=FALSE, critical.value=2){
	...
	res <- epMCA(DATA, make_data_nominal, DESIGN, make_design_nominal, masses, weights, hellinger, symmetric, correction, graphs, k)			
	...
	for(i in 1:test.iters){
		...
		perm.eigs <- permute.components(DATA,hellinger,symmetric,scaleFlag,centerFlag,correction)
...
#permute.components <- function(DATA,hellinger,symmetric,scaleFlag,centerFlag,correction){
#	perm.DATA <- apply(DATA,2,sample)
#	return(epMCA(perm.DATA,hellinger=hellinger,symmetric=symmetric,graphs=FALSE)$eigs)
#}