tepPLS <- function(DATA1,DATA2,center1=TRUE,scale1="SS1",center2=TRUE,scale2="SS1",DESIGN=NULL,make_design_nominal=TRUE,weights1=NULL,weights2=NULL,graphs=TRUE,k=0){
	...
	#apply(plsc.test$TExPosition.Data$lx,1,'*',plsc.test$TExPosition.Data$W)
	##we should probably include the weights into the factor scores across all methods.
	res$lx <- supplementalProjection(DATA1,res$fi * matrix(res$M,nrow(res$fi),ncol(res$fi),byrow=TRUE),Dv=res$pdq$Dv)$f.out
	res$ly <- supplementalProjection(DATA2,res$fj * matrix(res$W,nrow(res$fj),ncol(res$fj),byrow=TRUE),Dv=res$pdq$Dv)$f.out
